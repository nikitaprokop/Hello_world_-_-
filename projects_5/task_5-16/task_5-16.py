...
# ИСПРАВЛЕННЫЙ ЗАПРОС: используем p.id вместо p.product_id
query = """
SELECT
...
# Используем SQLAlchemy для подключения (рекомендуется pandas)
from sqlalchemy import create_engine

# Создаём engine для подключения
engine = create_engine('postgresql://postgres:student@localhost:5435/student_task')
